...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete PHOENIXSIV database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no PHOENIXSIV datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old PHOENIXSIV datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected PHOENIXSIV 0.7 datadir."
    ;;
  3)
    echo "Detected PHOENIXSIV pre-0.8 datadir."
    ;;
  4)
    echo "Detected PHOENIXSIV 0.8 datadir."
    ;;
esac
