PHOENIXSIV Core version *0.15.1* is now available from:

  <https://download.phoenixsiv.org/phoenixsiv-0.15.1/>
...
A number of changes to the way PHOENIXSIV Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 PSIV). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 PSIV) in the next release.
