...
const char * const BITCOIN_CONF_FILENAME = "phoenixsiv.conf";
const char * const BITCOIN_PID_FILENAME = "phoenixsivd.pid";
...
#else
    const char* pszModule = "phoenixsiv";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PHOENIXSIV";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PHOENIXSIV";
#else
    // Unix
    return pathRet / ".phoenixsiv";
#endif
#endif
}
